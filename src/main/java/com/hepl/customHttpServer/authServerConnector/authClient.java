...
public class authClient {
    ...
    private void command_receiver_decrypt()
    {

    }


    private String[] commandParser(InputStream rawInput) throws IOException {
        String[] buff = new String[2];
        byte[] commandType = new byte[2];
        int counter = 0, byteCounter = 0;
        int commandSizeBuffer = 0;
        while ((byteCounter = rawInput.read(commandType, byteCounter, 2 - byteCounter)) < 2) {
            byteCounter += byteCounter;
        }
        for (byte i : commandType) {
            buff[0] += Character.toChars(i);
        }

        // Reset
        commandType = new byte[4];
        counter = 0;

        while ((byteCounter = rawInput.read(commandType, byteCounter + byteCounter, 4 - byteCounter)) < 4) {
            byteCounter += byteCounter;
        }
        commandSizeBuffer = ((commandType[3] << 24) & 0xFF) + ((commandType[2] << 16) & 0xFF) + ((commandType[1] << 8) & 0xFF) + ((commandType[0]) & 0xFF);
        int commandSizeBufferWithoutHeader = commandSizeBuffer - 6;
        // Reset
        commandType = null;
        counter = 1;

        commandType = new byte[commandSizeBufferWithoutHeader];
        byteCounter = 0;
        while ((byteCounter = rawInput.read(commandType, byteCounter, commandSizeBufferWithoutHeader - byteCounter)) < commandSizeBufferWithoutHeader) {
            byteCounter += byteCounter;
        }
        try {
            AES.decrypt(commandType, key);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        for (int i = 0; i < commandSizeBuffer; i++) {
            buff[counter] += Character.toString((char) commandType[i]);
            if (Character.toString((char) commandType[i]).equals(",")) {
                counter++;
            }
        }
        return buff;
    ...
    //    private String[] command_Parser(InputStream rawInput) throws IOException
//    {
//        String[] buff = null;
//        byte[] command_type = new byte[2];
//        int counter = 0,byte_counter = 0;
//        int command_size_buffer = 0;
//        while((byte_counter = rawInput.read(command_type, byte_counter, 2 - byte_counter)) <= 2)
//        {
...
//        counter = 0;
//
//        while((byte_counter = rawInput.read(command_type, byte_counter + byte_counter, 4 - byte_counter)) <= 4)
//        {
//            byte_counter += byte_counter;
//        }
...
//        counter = 1;
//
//        while((byte_counter = rawInput.read(command_type, byte_counter + byte_counter, command_size_buffer_without_header - byte_counter)) <= command_size_buffer_without_header)
//        {
//            byte_counter += byte_counter;
//        }
//        try {
...
//    }
    public Customer getCustomer(String id) throws IOException
    {
        ...
        command_Parser = this.commandParser(inputStream);
        if(command_Parser[0].contentEquals("S"))
            Logger.log("Successful connection to auth_server");
        else {
            Logger.logWithGravity("Identification failed",Logger.Gravity.HIGH);
            return null;
        }
